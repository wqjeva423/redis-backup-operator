...
//			Name:      backupInstance.Name + "-" + backupInstance.Spec.ClusterName + "-job",
//		}
//
...
//				Namespace: req.Namespace,
//				Name:      backupInstance.Name + "-" + backupInstance.Spec.ClusterName + "-cronjob",
//			}
...
//				}
//			} else if err != nil {
//				msg := fmt.Sprintf("failed to get cronjob %s/%s error!", backupCronJob.Namespace, backupCronJob.Name)
//				log.Log.Error(err, msg)
//				return ctrl.Result{Requeue: true}, err
//			} else {
//				cronjob := utils.MakeCronJob(backupPvc, backupInstance.Spec.ClusterName, *backupInstance)
//				_, found, err := utils.ExistCronJob(cronjob.Name, cronjob.Namespace, r.Client)
//				if err != nil {
//					msg := fmt.Sprintf("found cronjob %s/%s error!", backupInstance.Namespace, backupInstance.Name)
//					log.Log.Error(err, msg)
//					return ctrl.Result{Requeue: true}, err
//				}
...
//					msg := fmt.Sprintf("rebuild cronjob %s/%s", cronjob.Namespace, cronjob.Name)
//					log.Log.Info(msg)
//					if err := r.Client.Delete(context.TODO(), cronjob, client.PropagationPolicy(metav1.DeletePropagationBackground)); err != nil {
//						msg := fmt.Sprintf("rebuild cronjob %s/%s error", cronjob.Namespace, cronjob.Name)
//						log.Log.Error(err, msg)
//						return ctrl.Result{Requeue: true}, err
//					}
